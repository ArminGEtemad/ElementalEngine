...
void respawnParticle(inout FireParticles p, uint id) {
    if (particleParams.slimeParticleCount == 0) {
        ...
        uint targetSlimeIdx = uint(seedHash * (float)particleParams.slimeParticleCount) % particleParams.slimeParticleCount;
    ...
    if (targetSlime.health <= 0.8f && targetSlime.health > 0.0f) {
        ...
        // Velocity X in
        p.velocity.x = targetSlime.velocity.x * 0.5f + (seed2 * 2.0f - 1.0f) * 25.0f;
