...
CSMain(uint3 DTid : SV_DispatchThreadID) {
  ...
  if (particleParams.lightningOpacity > 0.0001f) {
    ...
    if (dist2 < blastRadius2 && dist2 > 1e-4f) {
      ...
      float2 forceDir =
          toParticle / dist; // Radial vector pointing away from impact
      ...
      float impulseMag =
          forceStrength * falloff * particleParams.lightningOpacity;
      ...
      float ignitionDamage =
          falloff * particleParams.lightningOpacity * particleParams.dt;
