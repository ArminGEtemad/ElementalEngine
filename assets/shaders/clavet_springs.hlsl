...
void CSMain(uint3 DTid : SV_DispatchThreadID) {
    ...
    float2 springDisplacement = float2(0, 0);
        ...
        if (s.neighborID != INVALID_ID) {
            ...
            if (distSq > 1e-7f) {
                ...
                if (dist > s.restLength + toleDeform) {
                    // stretch
                    s.restLength += particleParams.dt * particleParams.plasticity * (dist - s.restLength - toleDeform);
                } else if (dist < s.restLength - toleDeform) {
                    // compress
                    s.restLength -= particleParams.dt * particleParams.plasticity * (s.restLength - toleDeform - dist);
                }
                ...
                float displacementMag = particleParams.dt * particleParams.dt 
                    * particleParams.springStiffness * fade * (s.restLength - dist);
                
                float2 rHat = r_ij / dist;
                
                //  x_i = x_i - D/2
                // and like before no need for the calculation for j since we are doing every cell
                    ...
                    if (dist2 < particleParams.interactionRadius2 && dist2 > 1e-7f) {
                        ...
                        for (uint k = 0; k < MAX_SPRINGS; k++) {
                            ...
                            if (springs[sIdx].neighborID == currNeighbor) {
                                springExists = true;
                                break;
                            } else if (springs[sIdx].neighborID == INVALID_ID && emptySlot == -1) {
                        ...
                        if (!springExists && emptySlot != -1) {
                            ...
                            // in the paper the restLength is h at this point but it did not show the behavior
                            // I liked when rendering. using dist instead of h worked more like the gooey and jelly
                            // that I wanted. 
    ...
    p.predictedPosition += springDisplacement;
    particles[id] = p;
}