...
struct Particle {
  ...
  uint32_t state; // 0: flying 1: settled (used to transition to the stam fluid)
  float pad0;
};
...
struct ParticleSimulationParameters {
  float dt;
  float restDensity;
  float stiffness; // resist to compression
  float viscosity;
  float particleMass;
  float smoothingRadius; // how far particles look for the neighbor
};
